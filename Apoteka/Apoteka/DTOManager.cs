...
namespace Apoteka
{
    public class DTOManager
    {
        ...
        /*
        public IEnumerable<Lek> GetLekovi()
        {
            ISession s = DataLayer.GetSession();

            IEnumerable<Lek> proizvodi = s.Query<Lek>()
        ...
        public Lek GetLek(string komNaziv)
        {
            ISession s = DataLayer.GetSession();

            return s.Query<Lek>()
                .Where(l => l.komercijalniNaziv == komNaziv).Select(p => p).FirstOrDefault();
        }*/
